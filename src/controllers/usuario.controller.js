...
async function perfil(req, res, next) {
  try {
    ...
    const usuario = await UsuarioService.buscarPerfil(req.usuario);
...
async function atualizarPerfil(req, res, next) {
  try {
    // Usamos o tipo e o id do token e os dados enviados no body.
...
async function removerMinhaConta(req, res, next) {
  try {
    // O service remove usando o tipo e o id que vieram do token.
    const resultado = await UsuarioService.removerMinhaConta(req.usuario);
